...
//   2. ~/.rlegend/policy.json          (user)
//   3. <cwd>/.rlegend/policy.json      (project — travels with the repo,
//                                         which is how cloud agents get it)
//   4. RLEGEND_POLICY env var pointing at a file (CI / managed push)
//
...
const { statePath, projectPolicyPaths } = require('./paths');

const BUNDLED = path.join(__dirname, '..', 'policy', 'policy.json');
const USER_POLICY = statePath('policy.json');

function layerPaths(cwd) {
  // The project layer is read under both the old and new directory names, in
  // that order, so a repo that committed `.vibeguard/policy.json` before the
  // rename keeps working and the new name wins where both exist. Dropping a
  // committed policy silently would quietly widen what an agent may reach.
  const out = [BUNDLED, USER_POLICY, ...projectPolicyPaths(cwd)];
  if (process.env.RLEGEND_POLICY) out.push(process.env.RLEGEND_POLICY);
  return out;
}
...
// Layers that may *widen* the safe-list. The bundled policy ships it, and a
// user editing their own home directory is making a decision about their own
// machine. The project layer travels with a checkout and $RLEGEND_POLICY can
// be set by anything, so neither gets to hand itself new auto-approvals — they
// can still narrow, which is the direction that is always safe.
function trustedLayers() {
  return new Set([BUNDLED, USER_POLICY]);
}

function load(cwd) {
  let policy = null;
  const sources = [];
  const trusted = trustedLayers(cwd);
  for (const file of layerPaths(cwd)) {
    const layer = readJson(file);
    if (!layer) continue;
    policy = policy ? merge(policy, layer, trusted.has(file)) : layer;
    sources.push(file);
  }
  if (!policy) throw new Error('rlegend: no readable policy');
  policy._sources = sources;
  return policy;
}

function merge(base, layer, trusted = false) {
  ...
  // Spreading would let any layer replace the whole block, which is the same as
  // letting it add entries. Merged explicitly, with the asymmetry in safelist.js.
  out.safeCommands = require('./safelist').mergeConfig(base.safeCommands, layer.safeCommands, trusted);
