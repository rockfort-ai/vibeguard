...
// call and get back a verdict plus the plain-English card that made VibeGuard
...
//   local    — the original VibeGuard rules about destructive local actions.
//
...
function decide(policy, call, ext) {
  ...
  if (unresolved) {
    return verdict(policy.defaults.unknownDestination, 'orange', 'egress.unresolved',
      `Talks to the network, but VibeGuard could not work out where (${unresolved.dest.unresolved}).`,
  ...
  if (unknown) {
    ...
    return verdict(policy.defaults.unknownDestination, 'orange', 'egress.unknown-destination',
      ...
      `Approve once, or run: vg allow ${d.host}`, ext);
  }
...
function classifyFileChange(filePath) {
  ...
  if (/\/\.(claude|cursor|codex|vibeguard)\//.test(p) && /(settings|hooks|config|policy)/.test(p))
