...
Requires the [Rockfort Legend Claude Code plugin](../plugins/rlegend), which does
...
cd extension
npm run package
code --install-extension rlegend-1.0.0.vsix     # VS Code
cursor --install-extension rlegend-1.0.0.vsix   # Cursor
```
...
|---|---|---|
| `rlegend.popupOn` | `orangeAndRed` | Which levels raise a popup. `all`, `orangeAndRed`, `red`, `off`. |
| `rlegend.revealPanelOn` | `red` | Which levels auto-open the panel. |
| `rlegend.statusBar` | `true` | Show the status bar indicator. |

## Commands

- **Rockfort Legend: Show recent activity** — open the panel
- **Rockfort Legend: Clear history**
- **Rockfort Legend: Check connection to Claude Code** — confirms verdicts are arriving
...
`RLEGEND_INTERACTIVE=1`. The hook then waits for your click and applies it.
...
It tails `~/.rlegend/events.jsonl`, written by the hook. It never runs
