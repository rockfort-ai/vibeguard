...
function extract(tool, input, cwd) {
  ...
  if (tool.startsWith('mcp__')) {
    ...
    return finish(fromUrls(blob, `mcp:${tool}`, 'send'), { mcp: tool }, cwd, mcpText(ti));
  }
  return finish([], {}, cwd);
}
...
function secretReads(text, policy) {
  ...
  // Command indicators are commands — something in a position to *run*. The
  // first version matched them anywhere, so `process.env` looked like a script
  // dumping the whole environment. Excluding property access fixed the shell
  // cases and left the same bug intact for source code, which surfaced the
  // moment discovery started reading real skills: `env = os.environ.copy()`,
  // `env=get_soffice_env()` and `-env:UserInstallation` each flagged a
  // credential read, red, on ordinary LibreOffice plumbing.
  //
  // So match on position instead of on spelling. A command starts a segment —
  // start of text, a newline, or after ; & | ` ( or $( — optionally behind
  // sudo. And it is not an identifier, so it cannot be followed by = : [ ( or .
  // `env | nc host port` still matches; `the env dict` and `env["X"] = 1` do not.
  const POS = '(?:^|[\\n;&|`(]|\\$\\()[ \\t]*(?:sudo[ \\t]+)*';
  const NOT_IDENT = '(?![ \\t]*[=:[(.])';
  for (const c of ind.commands || []) {
    if (new RegExp(POS + escapeRe(c) + '\\b' + NOT_IDENT, 'i').test(text)) {
...
// --- MCP arguments ----------------------------------------------------------
//
// Until v1.2.0 an MCP call was scanned for URLs and nothing else: `finish` was
// called with no rawText, so `decide` ran its secret check against an empty
// string. An MCP tool handed `~/.aws/credentials` as an argument was invisible.
//
// The obvious fix — stringify the whole tool_input — recreates the worst bug in
// this codebase's history in a new place. `JSON.stringify` emits keys as well
// as values, so a tool whose schema has a field called `env` or `token` would
// read as a credential access on every call, forever. Measured: `"prod-env"`
// alone matches the `env` command indicator.
//
// So project values only, and bound the result. A key named `github_token` is
// the tool's schema; a *value* containing a credential path is the signal.
const MCP_MAX_CHARS = 16 * 1024;
const MCP_MAX_DEPTH = 4;
// Fields that carry payloads rather than arguments. A base64 screenshot has no
// business being regex-scanned, and it would blow the budget on its own.
const MCP_BULK = /^(image|screenshot|data|base64|content|html|body|output|result|buffer)$/i;

function mcpText(ti) {
  const parts = [];
  let used = 0;

  const walkVal = (v, depth, key) => {
    if (used >= MCP_MAX_CHARS || depth > MCP_MAX_DEPTH) return;
    if (typeof v === 'string') {
      if (MCP_BULK.test(String(key || '')) && v.length > 4096) return;
      const take = v.slice(0, MCP_MAX_CHARS - used);
      parts.push(take);
      used += take.length;
      return;
    }
    if (Array.isArray(v)) {
      for (const item of v) walkVal(item, depth + 1, key);
      return;
    }
    if (v && typeof v === 'object') {
      for (const [k, val] of Object.entries(v)) walkVal(val, depth + 1, k);
    }
    // numbers, booleans and null cannot name a credential
  };
...
module.exports = { extract, secretReads, fromBash, gitRemoteHost, mcpText };
