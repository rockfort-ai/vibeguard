...
function hashSkill(files) {
  ...
  for (const f of files) {
    ...
    if (f.symlink !== undefined) h.update('symlink:' + String(f.symlink).replace(/\\/g, '/'));
      ...
      const bytes = canonBytes(f.rel, readBuf(f.path));
      // Per-file hash, kept on the entry so a pin can record it and drift can
      // later say *which* files moved instead of only that the skill did.
      f.h = crypto.createHash('sha256').update(bytes).digest('hex').slice(0, 12);
      h.update(bytes);
    }
    h.update('\0');
  }
...
// Hosts that appear in code as *identifiers*, not destinations. Every Office
// file library on earth carries the OOXML/ODF/XML namespace URIs as string
// constants; nothing ever connects to them. Flagging them taught this tool's
// own users to click through warnings — the calibration failure the rest of
// this file keeps warning about. Suffix-matched, and deliberately short: a
// host earns a place here by being a schema/namespace registry, nothing else.
const INERT_HOSTS = [
  'w3.org', 'openxmlformats.org', 'schemas.microsoft.com', 'schemas.google.com',
  'purl.org', 'purl.oclc.org', 'openoffice.org', 'ns.adobe.com',
  'xml.apache.org', 'relaxng.org', 'dublincore.org', 'xmlns.com', 'schema.org',
  'json-schema.org', 'yaml.org', 'iana.org',
  'example.com', 'example.org', 'example.net', 'localhost',
];

function inertHost(host) {
  const h = String(host).toLowerCase();
  return INERT_HOSTS.some((s) => h === s || h.endsWith('.' + s));
}

// A URL string cannot exfiltrate anything by existing; something has to make a
// request. So an *unrecognised* host in code is only a signal when the code
// also contains a network primitive. This is a corpus-level gate, not a
// proximity one — `URL = "https://evil.example"` used by a `requests.post`
// three hundred lines later still fires, but an XML library full of namespace
// constants and no sockets does not. The tokens are call-shaped on purpose:
// bare `urllib` would match `urllib.parse` in every parser ever written.
const NET_CONTEXT = new RegExp(
  [
    'requests\\.(get|post|put|patch|delete|head|request|Session)', 'urllib\\.request', '\\burlopen\\s*\\(',
    'http\\.client', '\\bhttpx\\b', '\\baiohttp\\b', 'urlretrieve',
    '\\bfetch\\s*\\(', '\\baxios\\b', 'XMLHttpRequest', 'new\\s+WebSocket',
    '\\bcurl\\s', '\\bwget\\s', 'Invoke-(WebRequest|RestMethod)',
    'Net::HTTP', '"net/http"', 'http\\.(Get|Post|PostForm)\\s*\\(',
    'socket\\.(socket|connect|create_connection)', 'net\\.(Dial|createConnection)',
  ].join('|'),
);
...
function signals(skill, policy) {
  ...
  // agent to POST somewhere works as well as code doing it. Never gated on
  // anything: a deny-listed host has no inert reading.
  const denied = [];
  ...
  // An *unrecognised* destination is only reported from code (prose is full of
  // links), only when it is not a namespace constant, and only when the code
  // can actually make a request. See INERT_HOSTS / NET_CONTEXT above.
  const canReachNet = NET_CONTEXT.test(code);
  const unknown = urlsIn(code).filter((d) => !inertHost(d.host) && !classify(policy, d));
  if (canReachNet) {
  ...
  // Secrets are scanned in code and prose alike — in prose a credential name is
  // a natural-language instruction to the agent, which is the whole SKILL.md
  // attack class. But the severity depends on where the data could *go*. A
  // file read or credential-dumping command is red on its own. A bare env-var
  // name is red only when the skill also has a denied or unrecognised network
  // path; naming the key you legitimately need (every API-calling skill does)
  // with nowhere unexpected to send it is a note, not an alarm.
  const secrets = secretReads(code + '\n' + prose, policy);
  if (secrets.length) {
    const names = [...new Set(secrets.map((s) => s.match))];
    const hard = secrets.some((s) => s.kind !== 'env');
    if (hard || denied.length || (canReachNet && unknown.length)) {
      add('red', 'skill.secret-reference',
        `References your credentials (${names.slice(0, 3).join(', ')})${denied.length || unknown.length ? ' and has a network destination outside your policy to send them to' : ''}.`);
    } else {
      add('orange', 'skill.secret-reference',
        `Names the credential ${names.slice(0, 3).join(', ')}, with no unrecognised network destination in its code — normal for a skill that calls the API it names. Worth a look only if this skill has no business needing it.`);
    }
  }
  ...
  // Raw observables, recorded at pin time so drift can later be described as a
  // *difference* — "one new destination" — instead of re-alarming on everything
  // the skill always did. Hosts are all of them, inert or not: the facts are a
  // baseline, not a judgement, and a judgement-free baseline is what makes the
  // later diff trustworthy.
  skill.facts = {
    hosts: [...new Set(urlsIn(code + '\n' + prose).map((d) => d.host))].sort(),
    secrets: [...new Set(secrets.map((s) => s.match))].sort(),
    scripts: scripts.map((f) => f.rel).sort(),
  };
...
// v3 adds `fileHashes` and `facts` to each pin, so drift can be described as a
// difference. v2 lockfiles load fine; their pins just can't be diffed until the
// next re-pin, and the renderer says so instead of pretending.
...
function pin(skills, lock, { accept = [] } = {}) {
  ...
  for (const s of skills) {
    ...
    lock.pinned[s.id] = {
      ...
      // What the reviewed bytes were, file by file, and what they did in the
      // aggregate. This is what turns the next drift report from "something
      // changed, panic" into "scripts/comment.py changed, no new destinations".
      fileHashes: Object.fromEntries(
        s.files.filter((f) => f.symlink === undefined && f.h).map((f) => [f.rel.replace(/\\/g, '/'), f.h]),
      ),
      facts: s.facts || null,
    };
  }
...
//
// "The hash moved" is true and useless — the realistic response to it is
// "accept" or "ignore", neither of which is review. What a person can actually
// judge is the delta: which files moved, and whether the skill gained a
// network destination, a credential reference, or a script it did not have
// when they approved it. A drift whose delta is empty of those is still drift
// (the bytes are unreviewed), but it is a different conversation from one that
// now phones somewhere new.
function driftOf(prev, skill) {
  if (!prev) return null;
  const out = { files: null, newHosts: [], newSecrets: [], newScripts: [], baseline: !!(prev.fileHashes && prev.facts) };

  if (prev.fileHashes) {
    const cur = new Map(
      skill.files.filter((f) => f.symlink === undefined && f.h).map((f) => [f.rel.replace(/\\/g, '/'), f.h]),
    );
    const added = [], removed = [], modified = [];
    for (const [rel, h] of cur) {
      if (!(rel in prev.fileHashes)) added.push(rel);
      else if (prev.fileHashes[rel] !== h) modified.push(rel);
    }
    for (const rel of Object.keys(prev.fileHashes)) if (!cur.has(rel)) removed.push(rel);
    out.files = { added, removed, modified };
  }

  if (prev.facts && skill.facts) {
    out.newHosts = skill.facts.hosts.filter((h) => !prev.facts.hosts.includes(h));
    out.newSecrets = skill.facts.secrets.filter((s) => !prev.facts.secrets.includes(s));
    out.newScripts = skill.facts.scripts.filter((s) => !prev.facts.scripts.includes(s));
  }

  // "Clean" is a statement about the delta only, never about the skill: no new
  // destinations, credential references, or runnable scripts versus the pin.
  out.clean = out.baseline && !out.newHosts.length && !out.newSecrets.length && !out.newScripts.length;
  return out;
}
...
function audit(cwd, policy) {
  ...
  const rows = skills.map((s) => {
    ...
    const sig = signals(s, policy); // also sets s.facts — must run before the spread
    return {
      ...s,
      status,
      ...
      signals: sig,
      drift: status === 'changed' ? driftOf(prev, s) : null,
    };
  });
        ...
        return {
          ...
          reason: drift
            ? 'its files changed since you pinned it'
              + (r.drift && r.drift.clean
                ? ' (no new network destinations, credential references, or scripts in the change)'
                : r.drift && r.drift.baseline
                  ? driftClause(r.drift)
                  : '')
...
// The part of a drift that matters, as the tail of a sentence. Empty when the
// delta introduced nothing — that case has its own, calmer phrasing.
function driftClause(d) {
  const bits = [];
  if (d.newHosts.length) bits.push(`now contacts ${d.newHosts.slice(0, 3).join(', ')}`);
  if (d.newSecrets.length) bits.push(`now references ${d.newSecrets.slice(0, 3).join(', ')}`);
  if (d.newScripts.length) bits.push(`added ${d.newScripts.length} script${d.newScripts.length > 1 ? 's' : ''}`);
  return bits.length ? ` — and the change ${bits.join('; ')}` : '';
}
...
module.exports = {
  discover, list, inspect, hashSkill, signals, audit, worstLevel, liveSignals, driftOf,
