...
// hooks/rlegend.js — which called bridge.emit() and bridge.waitForDecision() —
...
function sandbox() {
  const home = fs.mkdtempSync(path.join(os.tmpdir(), 'rlegend-bridge-'));
  fs.mkdirSync(path.join(home, '.rlegend', 'decisions'), { recursive: true });
  return home;
}

const events = (home) => {
  try {
    return fs.readFileSync(path.join(home, '.rlegend', 'events.jsonl'), 'utf8')
      .split('\n').filter(Boolean).map((l) => JSON.parse(l));
  } catch { return []; }
};

// The extension proves it is listening by touching a heartbeat file.
const beat = (home) => fs.writeFileSync(path.join(home, '.rlegend', 'extension.alive'), String(Date.now()));

// Stand in for a user clicking a button in the popup.
function answerWhenAsked(home, decision, reason) {
  const dir = path.join(home, '.rlegend', 'decisions');
...
async function run() {
  ...
  // 2. With an editor listening, an `ask` can be approved from the popup.
  {
    ...
    const r = await runHook(home, ASK_CALL, { RLEGEND_INTERACTIVE: '1', RLEGEND_TIMEOUT_MS: '4000' });
    stop();
  ...
  //    guarantee would be worth exactly one popup click.
  {
    ...
    beat(home);
    const stop = answerWhenAsked(home, 'allow');
    const r = await runHook(home, DENY_CALL, { RLEGEND_INTERACTIVE: '1', RLEGEND_TIMEOUT_MS: '1500' });
    stop();
    const d = r && r.hookSpecificOutput;
    const ev = events(home);
  ...
  //    than hanging the agent for the full timeout.
  {
    const home = sandbox();
    beat(home);
    fs.rmSync(path.join(home, '.rlegend', 'extension.alive')); // never answers
    const t = Date.now();
    const r = await runHook(home, ASK_CALL, { RLEGEND_INTERACTIVE: '1', RLEGEND_TIMEOUT_MS: '8000' });
  ...
  // 6. "Always allow" — the interruption budget. One answer, then silence.
  {
    ...
    const r1 = await runHook(home, ASK_CALL, { RLEGEND_INTERACTIVE: '1', RLEGEND_TIMEOUT_MS: '4000' });
    stop();
    ...
    const stored = JSON.parse(fs.readFileSync(path.join(home, '.rlegend', 'remembered.json'), 'utf8'));
