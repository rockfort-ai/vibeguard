...
| **What changed** — the delta, not just the hash | ✅ reliable | A pin records a hash per file plus the skill's destinations, credential references and script list. Drift is then reported as a difference: which files moved, and whether the change *added* a destination, a credential, or a script. A change that introduces none of those is said to introduce none of those. |
| **Execution out of a drifted skill** | ✅ **enforce** | `PreToolUse` deny, checked before the allowlist — an approved `Bash(python:*)` does not waive it, and it holds under `bypassPermissions`. The deny message carries the delta, so the decision is made on what changed rather than on the fact that something did. |
...
- **Two rules deliberately stay quiet, and both have a cost.** A URL in bundled
  code is only reported when the code also contains something that can make a
  request, and a short list of schema/namespace hosts (`w3.org`,
  `openxmlformats.org`, `schemas.microsoft.com`, `openoffice.org`, and similar)
  is never reported at all. Both exist because the alternative was measured: on
  an ordinary machine with the Office skills installed, the session-start report
  carried twelve "contacts an unrecognised host" lines, every one of them an XML
  namespace constant that nothing ever fetches. An alarm that is always wrong is
  not a conservative default — it is training to click through, and it spends
  the credibility the one true alarm needs. The cost is real and worth stating:
  a skill that keeps a URL as an inert constant and relies on the *agent* to
  fetch it is not reported by this rule, and a payload hosted under a
  namespace-registry hostname would be skipped. Deny-listed destinations are
  exempt from both rules and are always reported, wherever they appear.
- **Naming a credential is not the same as taking one.** A skill that references
  `ANTHROPIC_API_KEY` and whose only destination is `api.anthropic.com` is doing
  the thing it says on the tin; that is amber. It becomes red when the same
  skill also carries a blocked or unrecognised destination — the credential plus
  somewhere unexpected to send it — or when it reads credential *files* or runs
  a credential-dumping command, which are red on their own regardless of where
  the data could go.
