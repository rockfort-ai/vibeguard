...
function report({ rows, changed, added, risky, removed, firstRun, strict }) {
  ...
  if (changed.length) {
    ...
    for (const r of changed) {
      ...
      for (const line of driftLines(r.drift)) L.push(`      ${line}`);
      // With a per-file diff in hand, re-listing every standing signal is the
      // noise the diff exists to remove: only red ones still earn a line here.
      const sigs = r.drift && r.drift.baseline ? r.signals.filter((s) => s.level === 'red') : r.signals;
      for (const s of sigs) L.push(`      ${s.level === 'red' ? '!' : '-'} ${s.msg}`);
    }
    L.push(strict
...
// What actually changed, said as a difference. The old report proved the hash
// moved and left the reviewer to diff 15 scripts by hand — so nobody did.
function driftLines(d) {
  if (!d) return [];
  if (!d.baseline) {
    return ['pinned before file-level tracking — re-pin after review to enable precise diffs next time'];
  }
  const out = [];
  if (d.files) {
    const f = d.files;
    const part = (verb, list) => list.length
      ? `${verb} ${list.slice(0, 4).join(', ')}${list.length > 4 ? ` (+${list.length - 4} more)` : ''}` : '';
    const bits = [part('modified', f.modified), part('added', f.added), part('removed', f.removed)].filter(Boolean);
    if (bits.length) out.push(`what changed: ${bits.join('; ')}`);
  }
  if (d.clean) {
    out.push('the change introduces no new network destinations, credential references, or scripts');
  } else {
    if (d.newHosts.length) out.push(`! the change adds network destinations: ${d.newHosts.join(', ')}`);
    if (d.newSecrets.length) out.push(`! the change adds credential references: ${d.newSecrets.join(', ')}`);
    if (d.newScripts.length) out.push(`! the change adds runnable scripts: ${d.newScripts.slice(0, 4).join(', ')}`);
  }
  return out;
}
